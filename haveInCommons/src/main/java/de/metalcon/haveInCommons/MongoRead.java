...
public class MongoRead {
	...
	/**
	 * Setup connection to MongoDB server with appropriate parameters
	 * For debugging on local server instance all parameters are optional, just pass an empty string.
	 * Otherwise provide as many parameters as possible in the order described below.
	 * 
	 * @param connectionString - (host;[port][;database][;collection])
	...
	public MongoRead(String connectionString) throws UnknownHostException {
		if (connectionString.isEmpty()) {
			...
			if (args.length == 2) {
				...
				this.mongoClient = new MongoClient(new ServerAddress(args[0], Integer.parseInt(args[1])));
				this.db = this.mongoClient.getDB(args[2]);
	...
	/**
	 * 
	 * Insert a new value to given key as common entity. See wiki for documentation of underlying datamodel (pending).
	 * 
	 * @param key
	 * @param value
	...
	/**
	 * 
	 * Updates all matching entries which identified by key. If a entry with key does not exist, it will not be created.
	 *  
	 * @param key to update
	...
	private boolean updateCommons(String key, String value){
		...
		BasicDBObject pushCommand = new BasicDBObject("$push", new BasicDBObject("commons", value));
		
		// create new document if it does not exist: false
		// update all matching documents : true
		WriteResult result = this.collection.update(pushQuery, pushCommand, false, true);
		if(result.getN()==0)
			return true;
		else 
			return false;
	...
	public void delete(String value) {
		//TODO: Test if all items with occurences within key specifier will be matched and deleted!
		this.collection.remove(new BasicDBObject("key", value));
		
		BasicDBObject pullquery = new BasicDBObject("commons", value);
		this.collection.update(pullquery, new BasicDBObject("$pull", pullquery));
	}
}
