...
public class Server implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent arg0) {
		 ...
		 * 
		 * // create 5 users final long[] userIds = new long[5]; for (int i = 0;
		 * i < userIds.length; i++) { userIds[i] =
		 * this.graphDatabase.createNode().getId(); }
		 * 
		 * transaction.success(); transaction.finish(); transaction =
		 * this.graphDatabase.beginTx();
		 * 
		 * // create friendships final HashMap<Long, long[]> friendships = new
		 * HashMap<Long, long[]>(); friendships.put(userIds[0], new long[] {
		 * userIds[1], userIds[2], userIds[3], userIds[4] });
		 ...
		 * friendships.put(userIds[4], new long[] {}); for (long userId :
		 * userIds) { for (long followed : friendships.get(userId)) {
		 ...
		 * this.graphDatabase.beginTx();
		 * 
		 * // create status updates final SortedMap<Integer, Long> statusUpdates
		 * = new TreeMap<Integer, Long>(); // A statusUpdates.put(17,
		 * userIds[0]); statusUpdates.put(14, userIds[0]); statusUpdates.put(9,
		 * userIds[0]); // B statusUpdates.put(12, userIds[1]);
		 * statusUpdates.put(10, userIds[1]); statusUpdates.put(4, userIds[1]);
		 * // C statusUpdates.put(13, userIds[2]); statusUpdates.put(7,
		 * userIds[2]); statusUpdates.put(6, userIds[2]); // D
		 * statusUpdates.put(11, userIds[3]); statusUpdates.put(8, userIds[3]);
		 * statusUpdates.put(5, userIds[3]);
		 * 
		 * long timestamp; int message; while (!statusUpdates.isEmpty()) {
		 * timestamp = System.currentTimeMillis(); message =
		 * statusUpdates.firstKey();
		 * 
		 * this.graph.createStatusUpdate(timestamp,
		 * statusUpdates.remove(message), new
		 * PlainText(String.valueOf(message))); while
