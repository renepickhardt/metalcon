...
public class TestProcessCreateRequest {
	...
	public void testContainerRun() {
		...
		suggestTreeCreateRequestContainer.run(this.servletConfig
				.getServletContext());

		String retrieveResponse = this.servletConfig
				.getServletContext()
				.getAttribute(
						"indexName:" + ProtocolConstants.DEFAULT_INDEX_NAME)
				.toString(); // generalIndex.getBestSuggestions("test").toString();

		System.out.println(retrieveResponse);

		assertTrue(retrieveResponse
				.equals("{\"Status\":\"OK\",\"term\":\"testband\"}"));
	...
	//
	// ProcessCreateResponse response = new
	// ProcessCreateResponse(this.servletConfig.getServletContext());
	//
	//
	// if (response != null) {
